 ...
 */
export const KEYBOARD = (index: number): Device => index + 1;
/**
 * Gets the device ID of the `i`th mouse.
 * @param index The number of the mouse, 0-based, maximum: `MAX_MOUSE - 1`.
 */
export const MOUSE = (index: number): Device => MAX_KEYBOARD + index + 1;
...
export interface InterceptionNative {
     ...
     */
    getPrecedence(context: Context, device: Device): number;
    /**
     * Sets the precedence (priority) which this context wants for that specific device.
     */
    setPrecedence(context: Context, device: Device, precendence: number): void;

    getFilter(context: Context, device: Device): Filter;
    /**
     ...
     */
    wait(context: Context): Device | null;
    /**
